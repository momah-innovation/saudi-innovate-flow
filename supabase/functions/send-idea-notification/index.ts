...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const { 
      ...
      actionUrl, 
      metadata = {} 
    }: IdeaNotificationRequest = await req.json();

    console.log('Sending idea notification:', {
      ideaId,
      recipientId,
      notificationType,
      title
    ...
    return new Response(
      JSON.stringify({ 
        ...
        message: 'Idea notification sent successfully' 
      }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );

  } catch (error: any) {
    console.error('Error in send-idea-notification:', error);
    return new Response(
      JSON.stringify({ 
        success: false, 
        error: error.message 
