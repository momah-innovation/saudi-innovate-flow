-- ADDITIONAL SECURITY HARDENING: Address remaining security issues
...
FOR SELECT USING (
  ...
  EXISTS (
    ...
    WHERE itm.user_id = auth.uid() AND itm.status = 'active' AND itm.role = 'data_analyst'
  )
);
...
FOR SELECT USING (
  has_role(auth.uid(), 'super_admin'::app_role) OR
  has_role(auth.uid(), 'billing_admin'::app_role)
);

-- Log comprehensive security hardening
INSERT INTO public.security_audit_log (
  user_id, action_type, resource_type, details, risk_level
) VALUES (
  auth.uid(), 'COMPREHENSIVE_SECURITY_HARDENING', 'multiple_tables', 
  jsonb_build_object(
    'phase', 'secondary_hardening',
    'tables_secured', ARRAY['analytics_events', 'notifications', 'security_audit_log', 'file_records', 'user_roles', 'user_subscriptions'],
    'policies_applied', 12,
