-- PHASE 4B: Complete Database Migration - Part 2 (Additional Tables) - Fixed
-- Drop more check constraints and update remaining tables

-- Drop additional check constraints
ALTER TABLE challenge_requirements DROP CONSTRAINT IF EXISTS challenge_requirements_requirement_type_check;
ALTER TABLE challenge_scorecards DROP CONSTRAINT IF EXISTS challenge_scorecards_recommendation_check;
ALTER TABLE challenge_scorecards DROP CONSTRAINT IF EXISTS challenge_scorecards_risk_assessment_check;
ALTER TABLE challenge_shares DROP CONSTRAINT IF EXISTS challenge_shares_share_method_check;
ALTER TABLE challenge_live_presence DROP CONSTRAINT IF EXISTS challenge_live_presence_status_check;
ALTER TABLE ai_feature_toggles DROP CONSTRAINT IF EXISTS ai_feature_toggles_feature_category_check;
ALTER TABLE ai_feature_toggles DROP CONSTRAINT IF EXISTS ai_feature_toggles_required_subscription_tier_check;
ALTER TABLE ai_preferences DROP CONSTRAINT IF EXISTS ai_preferences_creativity_level_check;
ALTER TABLE ai_preferences DROP CONSTRAINT IF EXISTS ai_preferences_language_preference_check;
...
ALTER TABLE activity_events DROP CONSTRAINT IF EXISTS activity_events_privacy_level_check;
...
SET requirement_type = CASE requirement_type
  ...
  WHEN 'criteria' THEN 'requirement.criteria'
  WHEN 'validation' THEN 'requirement.validation'
