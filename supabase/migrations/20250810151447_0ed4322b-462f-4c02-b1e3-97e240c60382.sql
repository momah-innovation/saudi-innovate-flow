-- Fix Arabic translation keys by temporarily dropping all dependent policies
...
-- Recreate the dropped policies with correct column names
...
CREATE POLICY "Public can view published ideas" ON ideas
FOR SELECT USING (
  (status IN ('approved', 'in_development', 'implemented')) 
...
CREATE POLICY "Team members can view challenge partners" ON partners
FOR SELECT USING (
