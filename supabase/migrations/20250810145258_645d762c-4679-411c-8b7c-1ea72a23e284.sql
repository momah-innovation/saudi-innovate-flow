-- Temporarily disable idea validation trigger for migration
DROP TRIGGER IF EXISTS validate_idea_submission_trigger ON ideas;

-- PHASE 8: CRITICAL DATABASE VALUE MIGRATION (FINAL)
...
-- Step 2: Update ideas table (without trigger interference)
UPDATE ideas SET 
  status = CASE status
    WHEN 'draft' THEN 'status.draft'
...
CREATE TRIGGER validate_idea_submission_trigger
    ...
    FOR EACH ROW
    EXECUTE FUNCTION validate_idea_submission();
