...
CREATE TABLE IF NOT EXISTS public.expert_bookmarks (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
  expert_id UUID REFERENCES auth.users(id) ON DELETE CASCADE, -- Reference to expert user
  created_at TIMESTAMP WITH TIME ZONE DEFAULT now(),
  notes TEXT,
  priority VARCHAR DEFAULT 'medium',
  reminder_date TIMESTAMP WITH TIME ZONE,
...
-- Seed sample bookmark data
INSERT INTO public.focus_question_bookmarks (user_id, focus_question_id, notes, priority)
SELECT 
  u.id,
  fq.id,
  'سؤال مهم يحتاج للمتابعة',
  'high'
FROM auth.users u
CROSS JOIN (SELECT id FROM public.focus_questions LIMIT 2) fq
WHERE NOT EXISTS (
  SELECT 1 FROM public.focus_question_bookmarks fqb WHERE fqb.user_id = u.id AND fqb.focus_question_id = fq.id
)
LIMIT 10;

INSERT INTO public.campaign_bookmarks (user_id, campaign_id, notes, priority)
SELECT 
  u.id,
  c.id,
  'حملة مثيرة للاهتمام',
  'medium'
FROM auth.users u
CROSS JOIN (SELECT id FROM public.campaigns LIMIT 2) c
WHERE NOT EXISTS (
  SELECT 1 FROM public.campaign_bookmarks cb WHERE cb.user_id = u.id AND cb.campaign_id = c.id
)
LIMIT 10;

-- Enable realtime for all bookmark tables
...
ALTER TABLE public.team_bookmarks REPLICA IDENTITY FULL;
ALTER TABLE public.partner_bookmarks REPLICA IDENTITY FULL;
