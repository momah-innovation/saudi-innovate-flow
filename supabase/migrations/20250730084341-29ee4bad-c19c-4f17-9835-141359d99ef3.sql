-- First check the current constraint and update it to include our values
-- Update the challenge_type check constraint to include proper Arabic values
ALTER TABLE public.challenges DROP CONSTRAINT IF EXISTS challenges_challenge_type_check;

-- Add new constraint with proper Arabic challenge types
ALTER TABLE public.challenges ADD CONSTRAINT challenges_challenge_type_check 
CHECK (challenge_type IN ('تقني', 'أعمال', 'بيئي', 'صحي', 'تعليمي', 'technical', 'business', 'environmental', 'health', 'educational'));

-- Seed challenge data with proper challenge types
DO $$
BEGIN
