...
-- Create opportunity_bookmarks table (if not exists)
CREATE TABLE IF NOT EXISTS public.opportunity_bookmarks (
  ...
  created_at TIMESTAMP WITH TIME ZONE DEFAULT now(),
  CONSTRAINT fk_opportunity_bookmarks_opportunity_id 
    FOREIGN KEY (opportunity_id) REFERENCES partnership_opportunities(id) ON DELETE CASCADE,
  CONSTRAINT unique_opportunity_bookmark UNIQUE (opportunity_id, user_id)
);

-- Create opportunity_notifications table  
...
-- Enable RLS for opportunity_bookmarks
ALTER TABLE public.opportunity_bookmarks ENABLE ROW LEVEL SECURITY;

-- Create RLS policies for opportunity_bookmarks  
CREATE POLICY "Users can manage their own bookmarks" ON public.opportunity_bookmarks
  FOR ALL 
  USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);
...
CREATE INDEX IF NOT EXISTS idx_opportunity_comments_opportunity_id ON public.opportunity_comments(opportunity_id);
CREATE INDEX IF NOT EXISTS idx_opportunity_comments_user_id ON public.opportunity_comments(user_id);
CREATE INDEX IF NOT EXISTS idx_opportunity_comments_created_at ON public.opportunity_comments(created_at);
...
CREATE INDEX IF NOT EXISTS idx_opportunity_notifications_recipient_id ON public.opportunity_notifications(recipient_id);
CREATE INDEX IF NOT EXISTS idx_opportunity_notifications_opportunity_id ON public.opportunity_notifications(opportunity_id);
...
-- Create function to automatically refresh analytics when bookmarks change
...
-- Create trigger for bookmark changes
