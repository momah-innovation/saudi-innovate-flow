...
-- Create security event logging function (new)
...
-- Drop and recreate validate_role_assignment with new parameters
DROP FUNCTION IF EXISTS public.validate_role_assignment(uuid, app_role);

-- Enhanced role validation function (new parameters)
CREATE OR REPLACE FUNCTION public.validate_role_assignment(
  assigner_user_id uuid,
  target_role_value app_role
)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF has_role(assigner_user_id, 'super_admin'::app_role) THEN
    RETURN true;
  END IF;
  
  -- Admins can assign most roles except super_admin
  IF has_role(assigner_user_id, 'admin'::app_role) THEN
    RETURN target_role_value != 'super_admin'::app_role;
  END IF;
  
  -- Role managers can assign specific roles
  IF has_role(assigner_user_id, 'role_manager'::app_role) THEN
  ...
  IF has_role(assigner_user_id, 'team_lead'::app_role) THEN
    RETURN target_role_value = 'team_member'::app_role;
  END IF;
...
-- Enhanced has_role with better expiration support (new function name)
CREATE OR REPLACE FUNCTION public.has_role_enhanced(
  user_uuid uuid,
...
-- Comment: The existing has_role function is preserved to maintain compatibility with RLS policies