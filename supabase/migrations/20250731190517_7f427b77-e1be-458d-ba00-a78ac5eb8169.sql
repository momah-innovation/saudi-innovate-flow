-- Seed analytics tables with realistic data based on existing opportunities
DO $$
DECLARE
    opp_record RECORD;
    user_count INTEGER;
...
BEGIN
    ...
    SELECT COUNT(*) INTO user_count FROM opportunities WHERE status = 'open';
    
    -- For each opportunity, create realistic analytics data
    FOR opp_record IN (SELECT id FROM opportunities WHERE status = 'open' LIMIT 5) LOOP
        ...
        FOR i IN 1..FLOOR(RANDOM() * 30 + 20) LOOP
            ...
            INSERT INTO public.opportunity_view_sessions (
                ...
                END,
                NOW() - INTERVAL '1 hour' * RANDOM() * 720, -- Random time in last 30 days
                NOW() - INTERVAL '1 hour' * RANDOM() * 720 + INTERVAL '1 minute' * (RANDOM() * 45 + 5),
                FLOOR(RANDOM() * 8 + 1)::INTEGER,
                FLOOR(RANDOM() * 600 + 30)::INTEGER, -- 30 seconds to 10 minutes
    ...
    RAISE NOTICE 'Seeded analytics data for % opportunities', user_count;
END $$;