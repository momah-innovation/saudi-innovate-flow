-- Drop and recreate the get_bucket_stats function with correct return types
DROP FUNCTION IF EXISTS public.get_bucket_stats(text);

CREATE OR REPLACE FUNCTION public.get_bucket_stats(bucket_name text)
 RETURNS TABLE(total_files bigint, total_size bigint, avg_file_size numeric, oldest_file timestamp with time zone, newest_file timestamp with time zone)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  SELECT 
    ...
    CASE 
      WHEN COUNT(*) > 0 THEN COALESCE(AVG((metadata->>'size')::bigint), 0::numeric)
      ELSE 0::numeric
