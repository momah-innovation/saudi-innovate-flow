-- Seed user profiles and roles for the 20 users (using correct role types)

-- Insert profiles for all users using correct column names
WITH user_data AS (
...
SELECT 
  ...
  CASE 
    ...
    WHEN u.email LIKE 'mohammed.alharbi%' THEN 'content_manager'::app_role
    WHEN u.email LIKE 'sara.alzahrani%' THEN 'data_analyst'::app_role
    WHEN u.email LIKE 'khalid.almutairi%' THEN 'challenge_manager'::app_role
    WHEN u.email LIKE 'noura.alqahtani%' THEN 'domain_expert'::app_role
    WHEN u.email LIKE 'abdullah.alshamrani%' THEN 'domain_expert'::app_role
    WHEN u.email LIKE 'aisha.aldosari%' THEN 'content_manager'::app_role
    WHEN u.email LIKE 'omar.alghamdi%' THEN 'partnership_manager'::app_role
    ...
    WHEN u.email LIKE 'mansour.alahmed%' THEN 'domain_expert'::app_role
    WHEN u.email LIKE 'hind.alassaf%' THEN 'data_analyst'::app_role
    WHEN u.email LIKE 'fahad.alsubaie%' THEN 'user_manager'::app_role
    WHEN u.email LIKE 'reem.alkhalil%' THEN 'innovator'::app_role
    WHEN u.email LIKE 'sultan.alkhaldi%' THEN 'content_manager'::app_role
    WHEN u.email LIKE 'amira.alnajjar%' THEN 'domain_expert'::app_role
    WHEN u.email LIKE 'hassan.almasoud%' THEN 'challenge_manager'::app_role
...
-- Create expert profiles for users with domain_expert role
WITH expert_users AS (
  SELECT u.id, u.email, ur.role 
  FROM auth.users u
  JOIN user_roles ur ON u.id = ur.user_id
  WHERE ur.role IN ('domain_expert', 'evaluator') AND u.email LIKE '%@gov.sa'
)
...
WITH team_users AS (
  ...
  FROM auth.users u
  JOIN user_roles ur ON u.id = ur.user_id
  WHERE ur.role IN ('challenge_manager', 'content_manager', 'admin', 'data_analyst', 'user_manager') AND u.email LIKE '%@gov.sa'
)
...
SELECT 
  ...
  CASE 
    ...
    WHEN tu.email LIKE 'sara.alzahrani%' THEN 'تحليل البيانات'
    ELSE 'عضو فريق'
  END,
  CASE 
    WHEN tu.role = 'admin' THEN 'manager'
    WHEN tu.role = 'challenge_manager' THEN 'lead'
    ELSE 'member'
  END,
  CASE 
    WHEN tu.role = 'admin' THEN 5
    WHEN tu.role = 'challenge_manager' THEN 4
    ELSE 3
  END,
  ...
  CASE 
    ...
    WHEN tu.role = 'challenge_manager' THEN 70
    ELSE 60
  END
FROM team_users tu
ON CONFLICT (user_id) DO NOTHING;