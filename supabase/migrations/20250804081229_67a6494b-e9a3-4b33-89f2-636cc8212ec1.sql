-- Phase 3: Tag System Implementation (Fixed)
-- Adding tag system support for existing tables only
...
-- Create junction tables for many-to-many tag relationships (only for existing tables)
...
-- Create RLS policies for tag junction tables
CREATE POLICY "Everyone can view challenge tags" ON public.challenge_tags FOR SELECT USING (true);
