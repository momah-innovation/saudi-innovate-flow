-- Create a temporary function to handle system migrations
CREATE OR REPLACE FUNCTION public.create_idea_version_snapshot_migration_safe()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
    ...
    IF OLD.title_ar IS DISTINCT FROM NEW.title_ar OR 
       ...
       OLD.implementation_plan IS DISTINCT FROM NEW.implementation_plan THEN
        ...
        INSERT INTO public.idea_versions (
            ...
            END,
            COALESCE(auth.uid(), '00000000-0000-0000-0000-000000000000'::uuid), -- Use system UUID for migrations
            jsonb_build_object(
...
-- Replace the trigger with the migration-safe version
DROP TRIGGER IF EXISTS create_idea_version_snapshot ON ideas;
CREATE TRIGGER create_idea_version_snapshot
    BEFORE UPDATE ON ideas
    FOR EACH ROW
    EXECUTE FUNCTION create_idea_version_snapshot_migration_safe();
...
-- Restore the original trigger function after migration
