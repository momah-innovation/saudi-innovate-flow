-- Fix database relationships and add proper foreign key constraints

-- First, add proper foreign key constraints to existing tables
...
-- Migrate existing array data to junction tables
-- Campaign partners migration
INSERT INTO public.campaign_partner_links (campaign_id, partner_id)
SELECT c.id, unnest(c.partner_organizations)::uuid
...
-- Campaign stakeholders migration
INSERT INTO public.campaign_stakeholder_links (campaign_id, stakeholder_id)
SELECT c.id, unnest(c.target_stakeholder_groups)::uuid
FROM public.campaigns c
...
-- Event partners migration
INSERT INTO public.event_partner_links (event_id, partner_id)
SELECT e.id, unnest(e.partner_organizations)::uuid
FROM public.events e
WHERE e.partner_organizations IS NOT NULL 
AND array_length(e.partner_organizations, 1) > 0
ON CONFLICT (event_id, partner_id) DO NOTHING;

-- Event stakeholders migration
INSERT INTO public.event_stakeholder_links (event_id, stakeholder_id)
SELECT e.id, unnest(e.target_stakeholder_groups)::uuid
FROM public.events e
WHERE e.target_stakeholder_groups IS NOT NULL 
AND array_length(e.target_stakeholder_groups, 1) > 0
ON CONFLICT (event_id, stakeholder_id) DO NOTHING;

-- Event focus questions migration
INSERT INTO public.event_focus_question_links (event_id, focus_question_id)
SELECT e.id, unnest(e.related_focus_questions)::uuid
FROM public.events e
WHERE e.related_focus_questions IS NOT NULL 
AND array_length(e.related_focus_questions, 1) > 0
