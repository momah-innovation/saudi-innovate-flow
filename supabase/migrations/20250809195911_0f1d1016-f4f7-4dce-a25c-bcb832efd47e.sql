-- Create performance views only for existing tables
...
SELECT 
    ...
    COALESCE(oana.view_count, 0) as view_count,
    CASE 
        WHEN COALESCE(oana.view_count, 0) > 0 THEN 
            (COUNT(DISTINCT oa.id)::decimal / oana.view_count * 100)
        ELSE 0 
...
GROUP BY o.id, s.name_ar, d.name_ar, oana.view_count;

-- Campaign Overview View (without non-existing tables)
...
GROUP BY c.id, s.name_ar, d.name_ar, dep.name_ar;
...
-- Expert Overview View (simplified without non-existing tables)
...
LEFT JOIN expert_bookmarks eb ON e.id = eb.expert_id
GROUP BY e.id;

-- Partner Overview View (simplified without non-existing tables)
...
LEFT JOIN partner_bookmarks pb ON p.id = pb.partner_id
GROUP BY p.id;
...
SELECT 
    ...
    (SELECT AVG(engagement_rate) FROM challenge_analytics WHERE engagement_rate IS NOT NULL) as avg_challenge_engagement,
