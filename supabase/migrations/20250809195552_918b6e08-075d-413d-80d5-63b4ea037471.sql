-- Create performance views with correct column references
...
-- Event Overview View with corrected joins
...
-- Opportunity Overview View (fixed column references)
...
SELECT 
    ...
    AVG(ef.rating) as average_rating,
    COUNT(DISTINCT ef.id) as feedback_count,
    COALESCE(ea.consultation_count, 0) as consultation_count,
    COALESCE(ea.response_rate, 0) as response_rate,
    e.created_at,
    e.updated_at
FROM experts e
...
SELECT 
    ...
    COALESCE(pa.collaboration_count, 0) as collaboration_count,
