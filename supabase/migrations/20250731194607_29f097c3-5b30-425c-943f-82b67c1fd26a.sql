...
DELETE FROM opportunity_user_journeys 
WHERE opportunity_id NOT IN (SELECT id FROM opportunities);

DELETE FROM opportunity_live_presence 
WHERE opportunity_id NOT IN (SELECT id FROM opportunities);

-- Now fix foreign key constraints to reference opportunities table instead of partnership_opportunities
