-- Phase 1.1: Create Database Views for Admin Dashboard Metrics
...
SELECT 
  ...
  (SELECT COUNT(*) FROM user_roles WHERE role = 'expert' AND is_active = true) as expert_count,
  (SELECT COUNT(*) FROM user_roles WHERE role = 'partner' AND is_active = true) as partner_count,
...
-- 2. Create challenges_metrics_view for challenge statistics
...
SELECT 
  ...
  COUNT(*) FILTER (WHERE status = 'draft') as draft_challenges,
  COUNT(*) FILTER (WHERE status = 'evaluation') as evaluation_challenges,
  ...
  COUNT(*) FILTER (WHERE created_at > NOW() - INTERVAL '30 days') as new_challenges_30d,
  ...
  COUNT(*) FILTER (WHERE priority_level = 'critical') as critical_priority_count,
  ...
  -- Completion rate
  CASE 
    WHEN COUNT(*) FILTER (WHERE status IN ('active', 'published', 'completed')) > 0 
    THEN ROUND(
      COUNT(*) FILTER (WHERE status = 'completed')::NUMERIC / 
      COUNT(*) FILTER (WHERE status IN ('active', 'published', 'completed'))::NUMERIC * 100, 2
    )
    ELSE 0 
...
SELECT 
  ...
  (SELECT COUNT(*) FROM ideas) as ideas_table_size,
...
-- 5. Add RLS policies for the views
-- Note: Views inherit RLS from underlying tables, but we'll add explicit policies for admin access
...
-- Create RLS policies that allow only admins and team members to access these views
-- (Views will be queried through edge functions with proper authentication)