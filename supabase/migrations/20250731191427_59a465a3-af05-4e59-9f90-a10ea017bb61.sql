-- Continue seeding the remaining analytics tables
DO $$
DECLARE
    opp_record RECORD;
    session_id TEXT;
...
BEGIN
    FOR opp_record IN (SELECT id FROM public.opportunities WHERE status = 'open') LOOP
        ...
        -- Seed opportunity_geographic_analytics (match exact column structure)
        INSERT INTO public.opportunity_geographic_analytics (
            opportunity_id, country_code, country_name, region, city, view_count, first_view_at, last_view_at
        ) VALUES 
            (opp_record.id, 'SA', 'Saudi Arabia', 'Riyadh Province', 'Riyadh', FLOOR(RANDOM() * 200 + 20), NOW() - INTERVAL '30 days', NOW()),
            (opp_record.id, 'SA', 'Saudi Arabia', 'Makkah Province', 'Jeddah', FLOOR(RANDOM() * 150 + 15), NOW() - INTERVAL '25 days', NOW()),
            (opp_record.id, 'SA', 'Saudi Arabia', 'Eastern Province', 'Dammam', FLOOR(RANDOM() * 100 + 10), NOW() - INTERVAL '20 days', NOW()),
            (opp_record.id, 'AE', 'United Arab Emirates', 'Dubai', 'Dubai', FLOOR(RANDOM() * 80 + 8), NOW() - INTERVAL '15 days', NOW()),
            (opp_record.id, 'KW', 'Kuwait', 'Al Asimah', 'Kuwait City', FLOOR(RANDOM() * 60 + 6), NOW() - INTERVAL '10 days', NOW());
