...
async function fetchAdminMetrics(): Promise<AdminDashboardMetrics> {
  const [adminRes, systemRes, securityRes] = await Promise.all([
    queryBatcher.batch('admin_dashboard_metrics_view', async () =>
      supabase.from('admin_dashboard_metrics_view').select('*').maybeSingle()
    ),
    queryBatcher.batch('system_metrics_view', async () =>
      supabase.from('system_metrics_view').select('*').maybeSingle()
    ),
    queryBatcher.batch('security_metrics_view', async () =>
      supabase.from('security_metrics_view').select('*').maybeSingle()
    ),
  ]);

  const adminMetrics = adminRes.data as any;
  const systemMetrics = systemRes.data as any;
  const securityMetrics = securityRes.data as any;
  ...
  const metrics: AdminDashboardMetrics = {
    ...
    system: {
      uptime: 99.9,
      performance: 94,
      storageUsed: (systemMetrics?.total_storage_bytes || 0) / (1024 * 1024 * 1024),
      errors: 0,
      storage: {
    ...
    security: {
      ...
      riskLevel:
        securityMetrics?.security_score > 90
          ? 'low'
          : securityMetrics?.security_score > 70
          ? 'medium'
          : 'high',
...
export const useAdminDashboardMetrics = (
  ...
  const {
    ...
    queryKey: ['admin-dashboard-metrics', timeframe],
    queryFn: () => timeAsync(fetchAdminMetrics, 'admin-metrics-fetch'),
    staleTime: 5 * 60 * 1000,
    gcTime: 10 * 60 * 1000,
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      await queryClient.invalidateQueries({ queryKey: ['admin-dashboard-metrics', timeframe] });
      await refetch();
    } finally {
      setIsRefreshing(false);
    }
  }, [queryClient, refetch, timeframe]);
