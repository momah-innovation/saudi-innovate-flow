    ...
    try {
      const { data, error } = await supabase.functions.invoke('workspace-ai-assistant', {
        body: {
          workspaceId,
          workspaceType,
          query: t('workspace.ai.auto_analysis_query'),
          action: 'analyze',
          context: {
            autoGenerated: true,
      ...
      if (data?.success && data?.response) {
        ...
        if (aiResponse.insights) {
          aiResponse.insights.forEach((insight: string) => {
            generatedInsights.push({
              type: 'analytics',
              title: t('workspace.ai.performance_analysis'),
              description: insight,
              priority: 'medium',
              actionable: true
            })
        ...
        if (aiResponse.recommendations) {
          aiResponse.recommendations.forEach((rec: string) => {
            generatedInsights.push({
              type: 'suggestion',
              title: t('workspace.ai.improvement_suggestion'),
              description: rec,
              priority: 'high',
              actionable: true
            })
      ...
      setInsights([
        {
          type: 'analytics',
          title: t('workspace.ai.current_performance_analysis'),
          description: t('workspace.ai.fallback_performance_description'),
          priority: 'medium',
          actionable: true
        },
        {
          type: 'suggestion',
          title: t('workspace.ai.improvement_opportunity'),
          description: t('workspace.ai.fallback_improvement_description'),
          priority: 'high',
          actionable: true
        }
      ])
  ...
  const handleAIQuery = async () => {
    ...
    try {
      ...
      if (data?.success && data?.response) {
        ...
        switch (selectedAction) {
          case 'analyze':
            if (aiResponse.insights) {
              aiResponse.insights.forEach((insight: string) => {
                newInsights.push({
                  type: 'analytics',
                  title: t('workspace.ai.analysis_result'),
                  description: insight,
                  priority: 'medium',
                  actionable: true
                })
          ...
          case 'suggest':
            if (aiResponse.suggestions) {
              aiResponse.suggestions.forEach((suggestion: any) => {
                newInsights.push({
                  type: 'suggestion',
                  title: suggestion.title || t('workspace.ai.new_suggestion'),
          ...
          case 'summarize':
            if (aiResponse.summary) {
              newInsights.push({
                type: 'summary',
                title: t('workspace.ai.workspace_summary'),
          ...
          case 'optimize':
            if (aiResponse.optimizations) {
              aiResponse.optimizations.forEach((opt: any) => {
                newInsights.push({
                  type: 'optimization',
                  title: t('workspace.ai.optimization_for', { area: opt.area }),
        ...
        setQuery('')
        toast.success(t('workspace.ai.insights_generated_success'))
      }
    } catch (error) {
      console.error('AI query failed:', error)
      toast.error(t('workspace.ai.insights_generation_failed'))
    } finally {
      setIsLoading(false)
    }
  }
            ...
            {actionOptions.map(action => (
              <Button
                ...
                onClick={() => {
                  ...
                  setQuery(t('workspace.ai.quick_query_template', { action: action.label }))
                }}
              >
