      ...
      <div className="flex justify-between items-center">
        ...
        <Select value={timeRange} onValueChange={setTimeRange}>
          <SelectTrigger className="w-48">
            <SelectValue placeholder={t('challenge_analytics.select_time_period', 'اختر الفترة الزمنية')} />
          </SelectTrigger>
          <SelectContent>
                ...
                {analytics.topPerformers.map((challenge, index) => (
                  <div key={challenge.id} className="flex items-center gap-3 p-3 border rounded-lg">
                    ...
                    <div className="flex-1">
                      <p className="font-medium text-sm line-clamp-1" dir="rtl">{challenge.title}</p>
                      {challenge.title_en && (
                        <p className="text-xs text-muted-foreground line-clamp-1" dir="ltr">{challenge.title_en}</p>
                      )}
